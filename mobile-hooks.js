...
settings=function(){
  originalSettings();
  const audioRow=document.createElement('div');audioRow.className='mbtns';
  const soundButton=document.createElement('button');soundButton.className='sec';
  const soundLabel=()=>{soundButton.textContent=GameFeedback.muted?'Звук: выключен':'Звук: включён';soundButton.setAttribute('aria-pressed',String(!GameFeedback.muted));};
  soundLabel();soundButton.onclick=()=>{GameFeedback.toggle();soundLabel();};audioRow.append(soundButton);$('card').append(audioRow);
  const musicButton=document.createElement('button');musicButton.className='sec';
  const musicLabel=()=>{musicButton.textContent=GameFeedback.musicMuted?'Музыка: выключена':'Музыка: включена';musicButton.setAttribute('aria-pressed',String(!GameFeedback.musicMuted));};
  musicLabel();musicButton.onclick=()=>{GameFeedback.toggleMusic();musicLabel();};audioRow.append(musicButton);
